...
def dashboard(request:Request,db:Session = Depends(get_db)):
    ...
    total_revenue = db.query(Booking).with_entities(func.sum(Booking.price)).scalar() or 0 
    return templates.TemplateResponse('admin_dashboard.html',
                                      {'total_users':total_users,'total_buses':total_buses,
                                       'total_bookings':total_bookings,'total_revenue':total_revenue,'request':request})
...
async def add_bus_form(request:Request,db:Session = Depends(get_db)):
    ...
    form_data = {
        ...
        'bus_name':form.get('bus_name')
    }
    try:
        validated_data = NewBusForm(**form_data)
        print(validated_data)
        bus = Bus(
            bus_number = validated_data.bus_id,
        ...
        return templates.TemplateResponse("add_bus.html", {
            "request": request,
            "form_data": form_data,
            "errors": errors
        })
...
def add_bus(request:Request,db:Session = Depends(get_db)):
    ...
    print(places)
    return templates.TemplateResponse('add_place.html',{'request':request,'places':places})
...
async def add_place_form(request:Request,db:Session = Depends(get_db)):
    ...
    try:
        ...
        errors = {"__all__": str(e)}
        return templates.TemplateResponse("add_place.html", {
            "request": request,
            "form_data": form_data,
            "errors": errors
        })
...
async def list_buses(request: Request, db: Session = Depends(get_db)):
    buses = db.query(Bus).options(joinedload(Bus.routes).joinedload(BusRoute.place)).all()
    return templates.TemplateResponse("list_buses.html", {
        'request': request,
        'buses': buses
    })
...
async def add_route(request:Request,bus_id:int,db:Session = Depends(get_db)):
    ...
    new_stop_number = 1
    new_start_time=''
    last_stop = db.query(BusRoute).filter(BusRoute.bus_id == bus_id).order_by(BusRoute.stop_order.desc()).first()
    if last_stop: 
        new_stop_number = last_stop.stop_order + 1 
        new_start_time = last_stop.end_time.strftime("%Y-%m-%dT%H:%M")
    bus = db.query(Bus).filter(Bus.id == bus_id).first() 
    return templates.TemplateResponse("add_route.html",{
        'request':request,'places':places,'bus':bus,'new_stop_number' : new_stop_number,'new_start_time' : new_start_time
    })

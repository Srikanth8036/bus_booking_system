...
class SignupForm(BaseModel):
    ...
    @classmethod
    def as_form(
        cls,email:EmailStr = Form(...),username:str = Form(...),
        password:str = Form(...),phone_number:str = Form(...),address:str = Form(...),is_admin = Form(...)):
        return cls(email=email,username=username,password=password,phone_number=phone_number,address=address,is_admin=is_admin)
...
class NewBusForm(BaseModel):
    ...
    @classmethod
    def as_form(
        cls,bus_id:str = Form(...),bus_name = Form(...)):
...
class BusStopForm(BaseModel):
    ...
    def as_form(
        ...
        start_time_obj = time.fromisoformat(start_time.split('T')[1])  # Extract time from datetime string
        end_time_obj = time.fromisoformat(end_time.split('T')[1])  # Extract time from datetime string
